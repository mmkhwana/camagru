    ...
    if (isset($_GET['key']) && isset($_GET['email']))
    {
        ...
        try{
            ...
            if ($value === false)
            {
                ...
                $conn = new PDO("mysql:host=$servername;dbname=camagru", $dbusername, $dbpassword);
                $stmt_1 = $conn->prepare("UPDATE `users` SET verify = '1'  WHERE user_email = :user_email");
                $stmt_2 = $conn->prepare("UPDATE `users` SET user_key = 'NULL' WHERE user_email = :user_email");
